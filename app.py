...
async def play(interaction: discord.Interaction, query: str):
    ...
    embed.add_field(name="⏱️ Duration", value=str(datetime.timedelta(milliseconds=track.length)), inline=True)
    embed.add_field(name="📦 Position in Queue", value=str(len(vc.queue)), inline=True)
    embed.set_thumbnail(url=track.artwork)
    embed.set_footer(text=f"Requested by {interaction.user.name}", icon_url=interaction.user.display_avatar.url)
    await interaction.followup.send(embed=embed)

    if not vc.is_playing():
        await play_next(interaction.guild, vc, interaction.channel)
...
async def play_next(guild, vc, channel):
    ...
    now_playing.add_field(name="🎧 Author", value=track.author, inline=True)
    now_playing.add_field(name="⏱️ Duration", value=str(datetime.timedelta(milliseconds=track.length)), inline=True)
    now_playing.set_thumbnail(url=track.artwork)
    now_playing.set_footer(text="Enjoy your music 🎧")
...
# ---------- /stop ----------
@bot.tree.command(name="stop", description="Stop playback and leave the channel")
